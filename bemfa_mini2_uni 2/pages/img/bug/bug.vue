<template>
	<view>
		<view class="radio-list">
			<view style="color: #DD524D;"> 书本识别</view>
		...
		<view class="content">
			...
			<view class="txt" v-for="(item, index) in resItems" :key="item.value">
				<text>有 {{ (item.分类置信度 * 100).toFixed(1) }}% 概率是{{item.分类名称}}{{item.name}} </text>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	var typeUrl = 'http://4284316a52244299867c26d7843ff90a-cn-hangzhou.alicloudapi.com/ai_image_single_classification/ai_yingtao/yingtao/v1' //默认是动物识别
			...
			getResult: function() {
				...
				uni.request({
					...
					header: {
						...
						'Authorization': 'APPCODE 9097f44b888e4ace8669b6c655bce489'
					},
					success(res) {
						console.log(res.data)
						that.resItems = res.data.图像分类实体信息
						uni.hideLoading();
					}
				})
			}
